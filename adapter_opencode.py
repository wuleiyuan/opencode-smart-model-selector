...
class OpenCodeAdapter(BaseCLIAdapter):
    ...
    def parse_request(self, raw_input: Any) -> Dict[str, Any]:
        ...
        if isinstance(raw_input, str):
            # 字符串输入 - 直接作为任务描述
            return {"task_description": raw_input, "metadata": {"source": "cli_string"}}
        ...
        # 默认返回
        return {"task_description": str(raw_input), "metadata": {"source": "unknown"}}
    ...
    def format_cli_output(self, model: str, reason: str, verbose: bool = False) -> str:
        """
        格式化命令行输出
    ...
    def format_json_output(self, model: str, reason: str, metadata: Optional[Dict] = None) -> str:
        """
        格式化 JSON 输出
        ...
        Returns:
            JSON 字符串
        """
        output = {"model": model, "reason": reason, "platform": self.platform_name}
