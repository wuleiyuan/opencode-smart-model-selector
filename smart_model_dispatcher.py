...
class SmartModelDispatcher:
    ...
    def _stream_google(self, api_item, messages):
        ...
        for line in resp.iter_lines():
            if line:
                ...
                if decoded.startswith("data: "):
                    try:
                        text = json.loads(decoded[6:])["candidates"][0]["content"]["parts"][0][
                            "text"
                        ]
    ...
    def _stream_minimax(self, api_item, messages):
        url, headers = "https://api.minimax.chat/v1/chat/completions", {
            "Authorization": f"Bearer {api_item['key']}",
            "Content-Type": "application/json",
        }
        resp = self.session.post(
            url,
            headers=headers,
            json={"model": "MiniMax-M2.7", "messages": messages, "stream": True},
            stream=True,
            timeout=(10, 60),
        )
        resp.raise_for_status()
