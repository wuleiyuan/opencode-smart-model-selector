...
ZQ|| 🔀 **双引擎冗余** | 自定义调度引擎 + OpenCode 原生引擎，双重保障 |
SY|| ⚡ **熔断降级** | 连续3次失败自动切换到原生引擎，保证服务可用 |
XZ|| 📡 **API Server** | 提供 OpenAI 兼容接口，支持 Openclaw 集成 |
...
QZ|| `op auto / reset` | 清除手动指定，恢复智能模式 |
KH|| `op engine custom / native` | 切换双引擎模式 (自定义/原生) |
