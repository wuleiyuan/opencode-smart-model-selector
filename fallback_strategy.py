...
class DynamicFallback:
    ...
    def __init__(
        self, fallback_order: Dict[str, List[str]], settings: Dict[str, Any], telemetry=None
    ):
    ...
    def get_fallback(
        self, task_type: str, preferred_model: str, error_count: int = 0, avg_latency: float = 0.0
    ) -> str:
        """
        获取降级模型
    ...
    def _trigger_fallback(self, task_type: str, model_id: str, trigger: FallbackTrigger):
        """触发降级"""
        import time
    ...
    def _get_fallback_for_task(self, task_type: str, original_model: str) -> str:
        """获取任务的降级模型"""
        # 获取该任务类型的降级顺序
        fallback_list = self.fallback_order.get(task_type, self.fallback_order.get("default", []))
