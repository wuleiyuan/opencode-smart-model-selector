...
class PerformanceTelemetry:
    ...
    def _cleanup_old_data(self):
        ...
        self._data["selections"] = [
            e for e in self._data.get("selections", []) if e.get("timestamp", 0) > cutoff
        ]

        # 清理错误记录
        self._data["errors"] = [
            e for e in self._data.get("errors", []) if e.get("timestamp", 0) > cutoff
        ]
    ...
    def record_error(
        self, model_id: str, error_type: str, error_message: str, task_description: str = ""
    ):
        """记录错误"""
        if not self.enabled:
            return
