...
class OpenCodeDaemon:
    ...
    def check_api_health_detailed(self, api_key: str, base_url: str, provider: str) -> tuple:
        ...
        try:
            with requests.get(
                url, headers=headers, timeout=HEALTH_CHECK_TIMEOUT, proxies=self.proxy
            ) as response:
                # 只认可 200 为健康状态
    ...
    def health_check_loop(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                is_healthy, is_balance_insufficient = self.check_api_health_detailed(
                    api_key, base_url, current_provider
                )
                ...
                if is_balance_insufficient:
                    ...
                    logger.warning(
                        f"⚠️ 当前模型不健康: {current_provider} (连续失败: {consecutive_failures})"
                    )
